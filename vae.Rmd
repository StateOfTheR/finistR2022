...
On charge les données à partir du package **keras** et on les reformate
...
xtrain <- array_reshape(x_train, c(nrow(x_train), input_dim)) %>% torch_tensor
xtest <- array_reshape(x_test, c(nrow(x_test), input_dim)) %>% torch_tensor
print(dim(xtrain))
```
...
et la fonction de perte combine ici l'entropie croisée (pour le terme d'ajustement) et la pénalité Kullback Leibler :
...
```{r}
Res1 %>% as.matrix %>% as.data.frame() %>% mutate(Label = as.factor(ytrain)) %>%
```
...
vae$decoder(c(-2,3) %>% torch_tensor) %>% torch_reshape(c(28,28)) %>% as.matrix() %>% image
```
