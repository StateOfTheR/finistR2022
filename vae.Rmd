...
encoder.constructor <- nn_module(
  ...
  ## Définition des couches
    initialize = function(latent_dim) {
        self$conv = nn_conv2d(1, 4, kernel_size = 5, stride = 2)
        self$linear = nn_linear(576, 20)
    ...
    forward = function(input) {
        input = input %>%                           # N x 1 x 28 x 28
            self$conv() %>%                         # N x 4 x 12 x 12
            nnf_relu() %>% 
            torch_flatten(start_dim = 2) %>%        # N x 576
            self$dropout1() %>% 
            self$linear() %>%                       # N x 20
            nnf_relu() %>% 
            self$dropout2()
...
decoder.constructor <- nn_module(
  ...
  ## Définition des couches
    initialize = function(latent_dim) {
        self$linear = nn_linear(latent_dim, 576)
        self$unflatten = nn_unflatten(2, c(4, 12, 12))
        self$convt = nn_conv_transpose2d(4, 1, kernel_size = 5, 
                                         stride = 2, output_padding = 1)
        self$dropout = nn_dropout(.25)
    }, 
    ## Définitions des calculs
    forward = function(input) {
        input = input %>%                           # N x latent_dim
            ...
            nnf_relu() %>% 
            self$dropout() %>% 
            self$unflatten() %>%                    # N x 4 x 12 x 12
            self$convt() %>%                        # N x 1 x 28 x 28
            nnf_sigmoid()        
        input
    }
...
Res1 %>% 
  as.matrix %>% 
  as.data.frame() %>% 
  mutate(Label = as.factor(as.numeric(ytrain[1:10000]))) %>%
  ggplot(aes(x=V1,y=V2,col=Label)) + geom_point() + 
  scale_color_brewer(palette = "Paired")
```
