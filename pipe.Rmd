...
A noter une différence importante qui est la manière de gérer l'*overhead*, c'est à dire la position de l'argument `LHS` dans la fonction `RHS`.
...
Cette possibilité existe dans le pipe natif, mais **seulement** dans la version `R 4.2.+`. Et on utilisera la syntaxe `_` au lieu de `.`.
